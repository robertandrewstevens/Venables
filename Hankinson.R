...
# This example comes from Williams (1959), Example 2.3, page 20, 
# (with adaptations).
...
# Hankinson's formula relates the modulus of rupture, fµ, 
# for a timber plank to the angle of stress, µ:
  
# The formula may be "linearized" by taking reciprocals:
  
# but the error structure implied by fitting a simple regression 
# model to this formula may be unrealistic. It may be enough to 
# provide a starting estimate, though.

# A more realistic model might be to assume a multiplicative 
# error structure, that is, errors in the measurement of fµ 
# tend to affect the result proportionately rather than additively:
...
# Table 1: Modulus of rupture data for calibration of Hankinson's 
# formula
...
StartM <- lm(1/MRupture ~ 0 + I(cos(theta)^2) + I(sin(theta)^2), 
             hankinson)
(f <- structure(1/coef(StartM), names = c("a1", "a2")))

# We can now fit a non-linear model using these as starting 
...
# A generalization of the model allows the power to which the 
# trigonometric terms are raised to be a third parameter, 
# though such refinements are rarely necessary. To check this 
# in the present case, we fit the extended model and test the 
# simpler model as a sub-model of it.
...
# Table 3: Coefficient estimates from the extended non-linear 
# regression

anova(NonlinM, NonlinMg)

# Table 4: Analysis of variance table testing the standard 
# non-linear model, (1), within the extended model, (2)

# The main summary results are shown in Table 3 and the analysis 
# of variance results in Table 4.

# Now consider predictions from the standard model. First we set 
# up a data frame with the angle µ on a one degree scale. We use 
# the angle in degrees for plotting and the angle in radians for 
# computations.
...
# Note that simply back-transforming the predictions from the 
# log scale gives an estimate of the median modulus of rupture 
# on the natural scale, not the mean.

# For standard errors we consider two forms of bootstrap 
# estimates, first the so-called "Bayesian bootstrap", which 
# re-fits the model using random exponential weights. These are 
# chosen so that the weights have both mean and variance unity:
...
# Secondly, consider standard bootstrap estimates, where the 
# model is re-fitted using bootstrap samples of the original data:
...
with(pHankinson, {
  ...
  points(MRupture ~ thetaDeg, hankinson, pch = 4, 
         col = "darkgreen")
})
