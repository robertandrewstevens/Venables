...
The models are fully equivalent, but the coeficients refer to different things.
...
- Choosing W ~ Exp(1) gives E(W) = 1 = Var(W), the same as for the normal bootstrap. (Rubin gives a theoretical justification.)
...
- Using *data* to load a local ".csv" file; The **SOAR** package.

- **with** for local access to an object; **within** for modification of a data frame, with local access as well.

- **plot** methods for linear model objects.

- The **layout** function for multi-frame plotting.

- Mathematical expressions in plot annotation.

- The **make.link** function for generating non-standard links for GLMs; The **glm2** package for potentially troublesome GLM fits.

- Use of log scales in traditional graphics.

- Quick tests in polynomial regression using **poly**. Nested use of **with** for local access to two data frames simultaneously.

- The **replicate** convenience function for direct bootstrapping - the Bayesian Bootstrap idea of Rubin.
...
Shen, H. and Z. Zhu (2008). Effcient mean estimation in log-normal linear model. *Journal of Statistical Planning and Inference* 138, 552-567.
